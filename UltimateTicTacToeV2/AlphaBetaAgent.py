...
class AlphaBeta:
    ...
    def heuristic_ultimate_tic_tac_toe(self,board : Board, player : int):
        ...
        score += 5 * self.evaluate_local_board(board.miniBoard, player)#gives more wieght to the outter board 
    ...
    def get_input(self, events ,board: Board, train = None):
        _, move = self.AlphaBeta(board, self.player, self.maxDepth, board.last_played_place, -float('inf'), float('inf'))
        return move
    
    def AlphaBeta(self, board : Board, player : int, maxDepth : int, lastPlacePlayed : tuple =  (-1,-1),  alpha=-float('inf'), beta=float('inf')):
        ...
        if player == self.player:
            ...
            for move in LegalMoves:
                ...
                value, _= self.AlphaBeta(board_copy, -1 * player , maxDepth - 1, self.board.translate_place_to_otter_inner(move)[1], alpha, beta)
                if value > best_value:
                    best_value = value
                    best_move = move
